...
public class RequestController {
    ...
    *   view 페이지를 보여주는 방식
    *   1. String 타입 반환 값에 view 파일 이름 작성
    *   2. 메소드의 반환 타입을 void
    ...
    *   스프링 프레임워크는 내부적으로 Servlet 을 사용하고 있기 때문에
    *   HttpServletRequest 도 사용이 가능하다.
    *   다만 사용을 하지 않는 이유는 WebRequest 객체는
    *   Servlet 에 종속적이지 않으나 기능은 모두 포함하고 있기 때문에
    *   추후에 Servlet 기능이 사라졌을 시 적은 수정 범위로 교체할 수 있다는 장점이 있다.
    ...
    *   */
    ...
    public String registMenu(Model model, WebRequest request) {
        ...
        String message = menuName + "을(를) 신규 메뉴 등록" + categoryCode +
                " 번 카테고리에 " + menuPrice + "원으로 등록 했습니다!!!";
    ...
    *   name 속성이 일치하지 않을 떄 400-bad request 에러가 발생한다.
    ...
    *   */
    ...
    public String modifyMenu(Model model,
        ...
        String message = name + "의 가격을 "  + price + "원으로 수정했습니다!!!";
    ...
    *   */
    ...
    public String modifyAll(Model model,
        ...
        String message = menuName + "의 가격을 "  + modifyPrice + "원으로 수정했습니다!!!";
    ...
    *   하지만 받아 올 데이터가 많아진다면 관리할 변수나 키 값이 많아질 수 밖에 없다.
    *   @ModelAttribute 객체를 생성하여 요청되는 값을 필드와
    *   form 태그의 name 속성과 비교하여 값을 넣어준다.
    *   @ModelAttribute 로 담은 값은 view 페이지에서
    *   타입(자료형) 앞글자를 소문자로 한 네이밍 규칙을오 사용할 수 있다.(menuDTO)
    *   다른 이름을 사용하고 싶다면
    *   @ModelAttribute("사용할값") 이렇게 지정 할 수 있다.
    *   */
    @PostMapping("search")
